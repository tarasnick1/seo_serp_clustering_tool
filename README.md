...
serp.parquet (запрос → URL)
        ...
        ▼
┌──────────────────────┐
│ 1. Загрузка и кодирование │  StringEncoder: строки → int
│    (38 сек на 7.5M строк)  │
└──────────┬───────────┘
           ▼
┌──────────────────────┐
│ 2. Построение рёбер  │  Inverted index: URL → [запросы]
│    (77 сек)           │  Пара (A,B) → ребро если |SERP_A ∩ SERP_B| ≥ N
└──────────┬───────────┘
           ▼
┌──────────────────────┐
...
└──────────┬───────────┘
           ▼
┌──────────────────────┐
│ 4. Поиск клик        │  K-core split + fast_greedy
│    (147 сек)          │  352K сырых клик
└──────┬───────┬───────┘
       │       │
       ▼       ▼
┌──────────┐ ┌───────────────────┐
...
- Средний мульти-кластер: **5.7** запросов
- Максимальный: **1,302** запроса
...
Клики из Фазы 1 гарантируют только попарное условие. Но для SEO важно, чтобы существовали **конкретные URL-страницы**, которые ранжируются по ВСЕМ запросам кластера — это и есть потенциальные посадочные страницы.
...
| **Применение** | Широкая группировка тем | Точное определение посадочных |
...
- Финальной кластеризации для контент-плана
